...
void encode(string video_file, string encoded_file, int blockSize, int searchArea, int periodicity, int y_quant, int u_quant, int v_quant);
...
int main(int argc, char** argv)
{
	...
	if(opt=="-e" || opt=="--encode"){
		...
		encode(video_file, encoded_file, blockSize, searchArea, periodicity, y_quant, u_quant, v_quant);
	}
...
void printUsage(){
	cerr << "Usage:" 
		 << "\n\t" << "lossy_lhe -e|--encode <video_file> <encoded_file> <block_size> <search_area> <periodicity> <y_quant> <u_quant> <v_quant>" 
		 << "\n\t" << "lossy_lhe -d|--decode <encoded_file> <video_file>" 
		 << endl;
}
